...
const itemStyle = (done: any) => (done ? { bsStyle: 'success' } : {});
...
const ItemIcon = ( d: IItemIcon ) =>
  d.done ? <Glyphicon glyph="ok" className="item-done" /> : null;

// Renders a list of items, and a set of filter
// controls to change what's displayed in the
// list.
const FilteredList = (props: any) => (
  <section>
    ...
    {/* Renders the list of items. It passes the
         ...
         function is changed. */}
    <ListGroup>
      {props.items.filter(props.filter).map( (i: any) => (
        <ListGroupItem
          key={i.name}
